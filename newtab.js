...
// FIX: API key is no longer hardcoded. It's read from chrome.storage.sync
// so it never lives in source code. Set it via the Settings panel.
...
async function loadWeather() {
    ...
    // FIX: hide widget entirely if no key configured — no dead invisible space
    if (!apiKey) {
    ...
    navigator.geolocation.getCurrentPosition(
        async pos => {
            ...
            try {
                ...
                // Bail out cleanly on API error (bad key, quota, etc.)
                ...
                // FIX: hide widget on error — no dead invisible space
                document.querySelector(".weather-widget")?.style.setProperty("display", "none");
            }
        },
        // FIX: hide widget if geolocation is denied — no dead invisible space
        () => document.querySelector(".weather-widget")?.style.setProperty("display", "none")
    );
}
...
function clearCustomBackground() {
    ...
    // FIX: use removeAttribute("src") instead of src="" — the empty string
    // can be treated as a relative URL by some browsers, preventing reload.
...
function loadLinks(afterRender) {
    chrome.storage.sync.get(["links"], result => {
        ...
        // FIX: show empty state when there are no cards
        if (links.length === 0) {
            ...
            empty.innerHTML = `
                ...
                <span class="empty-sub">Right-click a card to edit, or add one via ⚙</span>
            `;
        ...
        links.forEach((link, index) => {
            ...
            // FIX: favicon fade-in — start invisible, reveal on load
            img.style.opacity = "0";
            img.style.transition = "opacity 0.3s ease";

            const revealImg = () => { img.style.opacity = "1"; };
            img.addEventListener("load",  revealImg);
            img.addEventListener("error", revealImg); // also reveal on broken icon
...
window.addEventListener("message", e => {
    // FIX: validate origin — only accept messages from our own extension,
    // preventing any web page from hijacking background changes
